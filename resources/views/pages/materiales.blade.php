@extends('layouts.main')

@section('title', '| Materiales')

@section('content')


<section class="service_grids color-dark bg-gray">
    <div class="container marginbot-50">
      <div class="row">

<br>
<br>
<br>
        <div class="col-lg-8 col-lg-offset-2">
            <div class="section-heading text-center">
            <div class="m_3"><span class="left_line1 wow fadeInLeft" data-wow-delay="0.4s"></span> <h1 class="h-bold wow fadeIn" data-wow-delay="0.4s">Materiales</h1><span class="right_line1 wow fadeInRight" data-wow-delay="0.4s"> </span></div>
            <div class="divider-header wow fadeInDown" data-wow-delay="0.4s"></div>
            <p class="wow fadeIn" data-wow-delay="0.4s">Materiales Disponibles Que Puedes Encontrar.</p>
         </div>
       </div>
     </div>
   </div>


<div class="text-center">
    <div class="container">
      <div class="row">
            <div class="col-md-3 ">
                <div class="wow fadeInLeft" data-wow-delay="0.2s">
                  <div>
                    <ul class="menu cf">
                      <li>
                        <a><img style="max-width:230px; margin-left:-5px;" src="../images/PAPELSolo.png" class="img-rounded" alt="imagen circular"></a>
                        <ul class="submenu">
                          <li><a href="tengo">Tengo</a></li>
                          <li><a href="necesito">Necesito</a></li>
                          <li><a href="como">¿Como?</a></li>
                        </ul>           
                      </li>
                    </ul>
                </div>
             </div>
          </div>

            <div class="col-md-3 ">
                <div class="wow fadeInLeft" data-wow-delay="0.2s">
                  <div>
                    <ul class="menu cf">
                      <li>
                        <a><img style="max-width:230px; margin-left:-5px;" src="../images/PLASTICOSolo.png" class="img-rounded" alt="imagen circular"></a>
                        <ul class="submenu">
                          <li><a href="tengo">Tengo</a></li>
                          <li><a href="necesito">Necesito</a></li>
                          <li><a href="como">¿Como?</a></li>
                        </ul>           
                      </li>
                    </ul>
                </div>
             </div>
          </div>

            <div class="col-md-3 ">
                <div class="wow fadeInLeft" data-wow-delay="0.2s">
                  <div>
                    <ul class="menu cf">
                      <li>
                        <a><img style="max-width:230px; margin-left:-5px;" src="../images/TELGOPORSolo.png" class="img-rounded" alt="imagen circular"></a>
                        <ul class="submenu">
                          <li><a href="tengo">Tengo</a></li>
                          <li><a href="necesito">Necesito</a></li>
                          <li><a href="como">¿Como?</a></li>
                        </ul>           
                      </li>
                    </ul>
                </div>
             </div>
          </div>




        </div>    
    </div>
</div>

</section>


<br>
<br>
<br>
<br>
<br>
<br>
<hr>
  <!-- /Section: services -->

@endsection
